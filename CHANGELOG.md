...
기호 처리와 관련하여 다수의 변경 사항을 반영하였습니다. Monoplex KR은 기호
155자가 늘고, Monoplex CJK는 그리스와 IPA 등 글자 184자가 빠집니다.
...
모든 수학 연산자(44자) 및 상당수의 도형(40자)을 다른 프로그래밍 글꼴의
경향에 맞추어 반각으로 변경하였고, 이 과정에서 잉크가 넘치는 경우는 가로세로
같은 비율로 줄였습니다. Plex Sans의 기호 중 Plex Mono로 다시 그릴 수 있는
경우 최대한 Mono의 글자체를 복원하였습니다. CJK 호환을 위한 기호 및 ❶ ⑴ 등의
둘러싼 문자는 전각으로 남겼습니다.

### 추가 변경 사항

- **143자를 뺐습니다.** 프로그래밍 글꼴에서 사용 빈도가 낮은 로마 숫자 및
딩벳, 보충 화살표 등을 삭제하였습니다.
- **칸을 넘던 16자를 같이 줄였습니다.** 반각에 할당되었으나 잉크가 반각을
넘치던 글리프를 모두 수정하였습니다.
- **SMALL 글리프를 구분되게 했습니다.** ∈, ∊와 같이 정상 크기와 작은 크기를
모두 갖는 경우, 작은 크기를 더욱 작게 만들어서 둘이 구분되게 하였습니다.
- **다양한 글꼴에서 온 기호, 도형의 크기와 높이를 맞췄습니다.** Plex Sans KR과 다른
계열은 글꼴 메트릭이 달라 기호 및 도형의 위치와 크기가 달랐습니다. 이를 모두
KR의 값에 맞추어 정리하였습니다. 기본적으로 KR의 글리프가 우선하였으나,
집합 연산자는 통일성을 위하여 모두 JP에서 가져와서 재정렬하였습니다.
- **둘러싼 숫자와 글자 176자를 ❶에 맞췄습니다.** ① ⑴ Ⓐ ⓐ ❶ 은 한 계열인데
두 겹 원 ⓵–⓾ 만 Plex Sans JP에서 와서 그것만 크고 높이 떠 있었고, 괄호를 두른
⑴ ⒜ 🄐 도 원과 줄이 맞지 않았습니다. 숫자에 마침표를 찍은 ⒈–⒛ 은 둘러싼 모양이
아니라 그대로 둡니다.
- **⸺ 과 ⸻ 을 이름대로 2em과 3em으로 만들었습니다.** 둘 다 전각 한 칸에 들어
있어 서로 구별되지 않았습니다. em 대시를 둘·셋 이어 붙여 반각 두 칸과 세 칸으로
다시 만들었습니다.
- **글자는 IBM Plex Mono에서만 가져옵니다.** 그리스, IPA 확장, 결합 분음 기호,
라틴 확장을 CJK 소스에서 받지 않습니다. CJK 소스의 그리스는 전각 칸에 맞춰 그린
것이라 대문자가 Plex Mono의 라틴보다 12% 높고 31% 넓어 반각 칸을 넘었고, 한
알파벳 안에서 일곱 자만 전각이었습니다. 결합 분음 기호는 26자가 폭이 0이 아니라
앞 글자에 얹히지 않고 제 칸을 차지했습니다. Monoplex CJK에서 184자가 빠져 시스템
글꼴로 넘어갑니다. 키릴 192자는 Plex Mono가 다 갖고 있어 그대로입니다.

### 두 가족의 기호를 같게 (Monoplex KR)

Monoplex KR은 Plex Mono와 Plex Sans KR의 기호만 포함하였으나, Monoplex CJK와의
호환성을 위하여 기호 영역이 동일하도록 확장하였습니다. Plex Sans JP·TC·SC를
소스로 쓰되 한자와 가나, 주음부호, 그리고 그것을 그리는 호환 문자를 받지 않습니다.
일본 문서에만 쓰는 〄 〠 〼 〽 〳 〴 〵 일곱 자도 Monoplex KR에서는 뺐습니다.

이제 두 계열의 차이는 한자와 가나입니다. 빌드할 때 받는 소스 글꼴이 24MB에서
175MB로 늘었고, 배포하는 글꼴 파일 크기는 거의 그대로입니다.

### 라이선스

라이선스 관련 내용이 부족하여 정정하고 보충했습니다. 저작권 표시와 라이선스,
상표 표시가 글꼴 파일의 name 테이블에 들어갑니다. ttf 하나만 따로 설치되어도
글꼴이 스스로 들고 다닙니다. Nerd Font 판에는 Nerd Fonts 쪽 표시가 더 붙습니다.

## 1.0.0 — 2026-09-20

0.0.2 이후 5년 만의 릴리즈입니다.
