...
기호 143자가 빠지고 여러 기호의 폭과 위치가 달라집니다.
...
- **143자를 뺐습니다.** 로마 숫자 Ⅰ–Ⅻ ⅰ–ⅻ, 기타 기술 블록의 ⌘와 괄호 조각
  ...
  시스템 대체 글꼴로 넘어갑니다.
- **❶–❿는 전각으로 둡니다.**
- **칸을 넘던 16자를 같이 줄였습니다.** ‿ ℏ ∉ ∟ ≃ ≅ ≢ ≲ ≳ ⊄ ⊅ ⊊ ⊋ ⊿ ⧺ ⧻ 는
  반각 칸을 받고도 크기가 줄지 않아 잉크가 칸을 12% 넘고 있었습니다. 1.0.0부터
  그랬고, 옆 글자가 줄면서 드러났습니다.
- **∊을 ∈과 다시 구분되게 했습니다.** ∈만 줄어 둘의 크기가 같아져 있었습니다.
- **∬과 ∭을 IBM Plex Mono의 ∫으로 다시 만들었습니다.** CJK 소스에서 오던 것을
  줄이다 보니 높이가 ∫의 77%와 56%까지 낮아져 있었습니다. ∫을 둘·셋 붙여
  가로로만 누르면 높이가 ∫과 같습니다. Monoplex KR에는 ∭이 새로 생깁니다.
- **⊕ ⊖ ⊗ ⊘ ⊙ ⊞ ⊠의 크기와 높이를 맞췄습니다.** ⊙만 세로가 길고 세로 중심이
  332~380으로 흩어져 있었습니다. 470 정사각으로 맞추고, `+`가 놓인 수학
  축(287)에 올렸습니다. 전에는 일곱 자 모두 그 축보다 46~94 위에 떠 있었습니다.
- **관계·연산 기호 21자를 수학 축에 올렸습니다.** ∅ ∦ ∧ ∨ ∩ ∪ ∴ ∵ ∷ ∼ ∽ ∝
  ≃ ≅ ≌ ≒ ≡ ≮ ≯ ⊥ ⋯ 가 `+` `−` `=` 보다 18~104 위에 떠 있었습니다. ∝는 짝이
  되는 ∞와 이제 같은 줄에 섭니다.
- **도형 40자의 크기와 높이를 맞췄습니다.** Sans KR에서 온 것은 470×528, Sans
  JP에서 온 것은 470×470으로 한 블록에 두 체계가 섞여 있었고 세로 중심도 갈려
  있었습니다. 큰 도형은 470 정사각으로, 40자 전부를 수학 축에 올렸습니다.
  ▶ ▷ ◀ ◁는 ▲ ▽보다 109 컸던 것을 맞췄고, ▪ ▫은 ■의 0.75배에서 0.6배로
  줄였습니다.
- **부분집합과 원소 계열을 IBM Plex Sans JP로 모았습니다** (Monoplex CJK).
  ⊂⊃⊆⊇와 ∈∋만 Sans KR에서 오고 나머지가 JP에서 와서 한 계열에 두 자형이
  섞여 있었습니다.
