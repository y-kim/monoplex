# 레시피 해설

`recipes/monoplex-kr.json` 과 `recipes/monoplex-cjk.json` 이 무엇을 하고
왜 그렇게 정했는지 적어 둔 문서입니다. 레시피의 `$note` 에 흩어져 있는
근거를 한자리에 모은 것이라, 레시피를 고치기 전에 읽으시면 됩니다.

글꼴을 쓰는 쪽에서 겪는 것은 [CHANGELOG.md](CHANGELOG.md)에 있습니다.

- 레시피를 돌리는 법은 [HOW_TO_BUILD.md](HOW_TO_BUILD.md)
- 버전을 매기는 기준은 [VERSIONING.md](VERSIONING.md)
- 릴리즈별 변경 이력은 [CHANGELOG.md](CHANGELOG.md)

## 메트릭

네 가족이 모두 같습니다. 섞어 써도 줄이 맞습니다.

| | 값 |
|---|---|
| unitsPerEm | 1000 |
| 좁은폭 (라틴·기호) | 528 |
| 넓은폭 (한글·한자·가나) | 1056 |
| hhea ascent / descent | 950 / −225 |
| typoLineGap | 80 |
| italicAngle | −9 |
| OS/2 xAvgCharWidth | 528 |
| post.isFixedPitch | 1 |
| PANOSE | 2 11 ? 9 5 2 3 0 2 3 |

PANOSE의 세 번째 자리(weight)만 굵기마다 다릅니다.

0.0.2에서 1.0.0으로 오면서 이 값들은 하나도 바뀌지 않았습니다. 줄바꿈
위치와 세로 정렬이 그대로인 이유입니다.

### PANOSE 수정

상류 PlemolJP(4143f3d)의 수정을 가져왔습니다.

| 자리 | 전 | 후 |
|---|---|---|
| bContrast | 2 | 5 |
| bLetterForm | 2 | 0 |
| bXHeight | 7 | 3 |

bProportion은 9(Monospaced)로 둡니다.

## 공백 폭

전각 1056의 정수 분할로 맞췄습니다. 0.0.2까지는 IBM Plex Mono가 고정폭
글꼴이라 원본부터 모두 600이었고, 축소를 거쳐 전부 528이 되어 있었습니다.
그래서 `U+2005 FOUR-PER-EM`이 이름과 달리 em의 1/2였습니다.

| 유닛 | 문자 |
|---|---|
| 1056 | U+2001 EM QUAD, U+2003 EM SPACE, U+3000 IDEOGRAPHIC SPACE |
| 528 | U+0020 SPACE, U+00A0 NBSP, U+2000 EN QUAD, U+2002 EN SPACE, U+2007 FIGURE, U+2008 PUNCTUATION |
| 352 | U+2004 THREE-PER-EM |
| 264 | U+2005 FOUR-PER-EM, U+205F MEDIUM MATHEMATICAL |
| 176 | U+2006 SIX-PER-EM, U+2009 THIN, U+202F NARROW NBSP |
| 132 | U+200A HAIR |

이름과 유니코드 정의가 어긋나는 곳이 셋 있습니다. `U+2005`는 정의상
em의 1/4이므로 264로, `U+2009 THIN`은 1/5이나 인접한 약수인 1/6(176)으로,
`U+200A HAIR`는 1/10이나 1/8(132)로 맞췄습니다. 전각의 약수여야 칸이
어긋나지 않습니다.

레시피에는 유닛이 아니라 분모로 적습니다. 전각 폭을 바꿔도 따라갑니다.

## 이름과 스타일 비트

### macStyle과 fsSelection

0.0.2까지 `head.macStyle`을 아무도 채우지 않아 16개 중 10개가
`fsSelection`과 어긋나 있었습니다. 값이 어긋나면 일부 앱이 진짜 이탤릭
위에 가짜 기울임을 덧씌웁니다. ttfautohint가 빌드마다 경고하던 것이
이것입니다.

`fsSelection`은 파일 이름에 `Bold`가 들어 있는지로 정하고 있었습니다.
그래서 SemiBold가 BOLD 비트를 달았고, 글꼴 선택기에서 SemiBold가 그
가족의 볼드로 잡혀 Bold와 경쟁했습니다.

이제 둘 다 스타일 표에서 유도합니다. Regular와 Bold만 한 패밀리의 네 칸
(RIBBI)을 쓰고 나머지 굵기는 별도 패밀리를 가지므로, SemiBold는
"Monoplex KR SemiBold" 패밀리의 Regular이지 Bold가 아닙니다.

### Nerd Font 판의 이름

`Monoplex KR Nerd`에서 `Monoplex KR Nerd Font`로 바꿨습니다. Nerd Fonts의
패처가 기본으로 붙이는 형태이고, 도구들이 이 이름으로 글꼴을 찾습니다.
라이선스가 강제하는 것은 아니지만 관례를 따르는 편이 낫습니다.

## 소스 우선순위

같은 코드포인트를 여러 글꼴이 갖고 있을 때 앞에 있는 것을 씁니다. 두 가족이
같은 사슬을 씁니다.

```
IBM Plex Mono  →  Plex Sans KR  →  Plex Sans JP  →  Plex Sans TC  →  Plex Sans SC
   라틴              한글            가나·한자         한자 보충        간체·확장A
```

Monoplex KR은 여기서 한자와 가나를 뺀 것입니다. JP·TC·SC를 소스로 쓰되 한자,
가나, 주음부호와 그것을 그리는 호환 문자를 `dropRanges`로 막습니다. 々 〆 〇,
칸분 ㆐–㆟, 괄호·원문자 한자 ㈠–㉇ ㊀–㊰, 달·시각·날짜 전신 기호, 레이와 ㋿,
원문자 가타카나 ㋐–㋾, 제곱 가나말 ㌀–㍗, 제곱 연호 ㍻ ㍼ ㍽ ㍾ 와 ㍿ 이
그것입니다. 원문자 숫자 ㉑–㊿ 와 제곱 단위 ㎏ ㎝ ㎞ 는 한자도 가나도 아니라서
남깁니다. 일본 문서에만 쓰는 〄 〠 〼 〽 〳 〴 〵 일곱 자도 Monoplex KR에서는
뺍니다.

전에는 Monoplex KR이 Mono 와 Sans KR 만 써서, 같은 계열의 기호가 Monoplex CJK
에는 있고 KR 에는 없는 일이 있었습니다. 이제 기호는 두 가족이 같고, 함께 가진
글자는 폭도 자형도 한 자도 다르지 않습니다. 대신 Monoplex KR 을 빌드할 때도
소스 글꼴을 175MB 받습니다. 배포하는 글꼴 파일 크기는 거의 그대로입니다.

일본어 글자는 모두 JP에서 가져옵니다. 한자에 이체자가 있을 때 JP를 TC·SC
앞에 둔 것은, 일본 쪽에 저장된 자형이 한국에서 쓰는 자형과 더 잘 맞기
때문입니다.

호환한자(U+F900–FAFF)는 소스 글꼴에 없습니다. 같은 한자의 다른 독음을
유니코드가 따로 부호화한 것이라 자형이 통합한자 쪽과 같으므로, 정준
등가(canonical decomposition)로 cmap을 연결했습니다. 그래서 KS X 1001의
한자 4,888자(통합 4,620 + 호환 268)를 100% 덮습니다.

### TC의 사용자 정의 영역

IBM Plex Sans TC는 PUA에 벤더 내부용 글리프를 4,729자 갖고 있습니다.
우선순위상 이것이 Nerd Fonts보다 앞이라 Nerd Fonts 영역을 통째로
가로챘습니다. Powerline 구분자 자리에 TC의 내부 글리프가 들어앉아 폭이
전각 1056이 되고, 터미널에서 칸이 어긋났습니다.

JP·TC·SC에서 PUA를 제외하니 Nerd Fonts의 기여가 8,307자에서 10,868자로
늘었습니다. Monoplex KR 계열에서는 드러나지 않던 문제입니다 —
IBM Plex Sans KR은 PUA가 0자입니다.

## 세로 정렬

Plex Sans의 한자와 가나는 한글과 세로 위치가 다릅니다. 섞어 쓰면 한자만,
또 가나만 들려 보입니다. 둘 다 내려서 맞췄습니다.

**한자** — 자면 중심이 한글보다 75유닛 위에 있었습니다. 최대 자면 기준으로
한글 304.5 / 한자 380이었고, 보정 후 310.9 / 309.9입니다.

**가나** — 위끝이 한자보다 40유닛 높았습니다.

| | 위끝 (보정 전) | 위끝 (보정 후) |
|---|---|---|
| 한자 | 772 | 772 |
| 히라가나 | 815 | 775 |
| 가타카나 | 809 | 769 |
| 반각 가타카나 | 816 | 770 |
| 한글 | 769 | 769 |

재는 방법: 탁점·반탁점이 붙은 글자(`が` `パ`)는 그 점이 자면 위로 올라가는
것이 정상이라 뺐습니다. 그리고 `鬱` 같은 극단적인 글자가 최대값을 끌어
올리므로 최대 대신 상위 5%를 썼습니다.

`、。「」・` 같은 CJK 구두점은 위끝이 760으로 원래 한자와 맞아 있어서
건드리지 않았습니다. 장음부 `U+30FC ー`는 가나의 일부로 읽히므로 가나와
함께 내렸습니다.

둘 다 평행이동입니다. 자형 크기와 획 굵기는 손대지 않았습니다.

### 가로는 왜 안 맞추는가

한자의 최대 자면 가로는 934, 한글은 857로 한자가 9% 넓습니다. 세로는
922 대 930으로 이미 1% 안쪽입니다.

가로만 맞추려고 한쪽을 한 축으로만 늘이거나 줄이면 세로획과 가로획의
굵기 비율이 깨집니다. 한글을 가로로 8% 늘여 봤더니 세로획만 8% 굵어졌고,
한자를 가로세로 함께 9% 줄여 봤더니 가로는 맞았지만 세로가 0.903으로
어긋나면서 애써 맞춘 세로 위치까지 위로 떴습니다.

원래 자형의 비율이 가장 정확하므로 그대로 두었습니다. 한자가 9% 넓은
것은 IBM Plex Sans 자체의 설계입니다.

## 이탤릭

라틴과 한글만 기울입니다. 한자와 가나는 이탤릭 굵기에서도 곧게 둡니다.

라틴은 IBM Plex Mono의 실제 이탤릭 파일을 쓰고, 한글은 합성입니다
(`italicize`, −9도). CJK는 전통적으로 이탤릭이 없어 기울이면 어색합니다.
덧붙여 FontForge의 `italicize`는 글리프마다 윤곽을 다시 계산해서, 한자
2만 자에 돌리면 이탤릭 굵기가 정체보다 몇 배 느려집니다.

## Nerd Fonts

v2.0.0에서 v3.5.1로 올렸습니다. v3에서 Material Design Icons가
U+F500–FD46에서 U+F0001–F1AF0으로 옮겨갔습니다.

소스는 Symbols-only 대신 Blex Mono Nerd Font를 씁니다. IBM Plex Mono
패치본이라 전 글리프 폭이 600으로 균일해서 기존 파이프라인의 전제와
그대로 맞습니다.

넣을 코드포인트 범위는 상류의 목록을 옮겨오지 않고
`cmap(BlexMonoNerdFont 3.5.1) − cmap(IBMPlexMono)`로 직접 산출했습니다.
검증 결과 누락 0, 초과 0입니다.

**Pomicons(U+E000–E00A)는 제외합니다.** 상업적 이용이 제한되는
라이선스입니다.

### Powerline 구분자

v2의 구분자는 자면이 칸 폭에 모자라서(U+E0B0이 0..528/600) 코드에서 밀어
맞췄습니다. v3.5.1은 −36..599로 칸을 넘치게 설계되어 있어 그 보정이
오히려 칸 사이에 틈을 만듭니다. 제거했습니다.

## 기호 폭

이슈 #2의 정리입니다. 수학 기호와 도형, 화살표 같은 기호 블록에서 반각과
전각이 섞여 있었습니다. IBM Plex Mono가 가진 기호는 반각, IBM Plex Sans KR·JP·
TC·SC가 가진 기호는 그 글꼴이 전각으로 그리므로 전각으로 들어왔기
때문입니다. 터미널(xterm.js, Windows Terminal)은 East Asian Width가 W나 F가
아닌 글자를 1칸으로 세므로, 전각 글리프는 옆 칸을 침범합니다.

글자마다 검토해서 셋으로 나눴습니다. 결정 근거로 다른 고정폭 글꼴 13종
(나눔고딕코딩, D2Coding, JetBrains Mono, Liberation Mono, Cascadia Code, Hack,
Cousine, Fira Code, Consolas, JuliaMono, Source Code Pro, IBM Plex Mono,
Sometype Mono)의 폭을 함께 봤습니다.

| 결정 | 글자 수 | 처리 |
|---|---|---|
| 반각 | 110 | `narrowToHalf`. 잉크 폭이 470을 넘으면 가로세로 같은 비율로 줄이고 반각에 가운데 놓는다. 겹문장부호 5자는 아래처럼 조합 |
| 전각 | 10 | ❶–❿ (U+2776–277F). 그대로 둔다 |
| 제거 | 143 | `finalize.removeCodepoints`. 로마 숫자, 기타 기술(⌘, 괄호 조각), 기타 기호(★ ♥ ☎ 등), 딩뱃 일부, 보충 화살표 |

기준 470은 IBM Plex Mono 자체 반각 기호의 잉크 폭(중앙값 436, 상위 5% 513)에서
잡았습니다. 원 모양(⊙ ○ ◎)은 가로만 누르면 뜻을 잃으므로 같은 비율로
줄입니다. ⊙은 62%가 됩니다.

겹문장부호 ‼ ⁇ ⁈ ⁉와 ‥는 예외입니다. 통째로 줄이면 크기가 제각각이고
너무 작아져서, IBM Plex Mono의 `!` `?` `.`를 붙여 만듭니다(`compose` op).
‼는 `!` 두 개를 그대로, ⁇는 `?`를 가로로만 58%로 좁혀 두 개, ⁈와 ⁉는
65%로 좁힌 `?`와 그대로의 `!`, ‥는 `…`의 점 간격으로 `.` 두 개입니다. 높이가
본문 부호와 같아집니다. Hack이 같은 방식이고, Plex Mono의 `?`가 Hack보다
넓어 비율을 더 줄였습니다.

같은 방법으로 다섯 자를 IBM Plex Mono에서 만듭니다. ‵(U+2035)는 ′를 좌우로,
∇(U+2207)는 ∆(U+2206)를 상하로, ℧(U+2127)는 Ω(U+2126)를 상하로 뒤집은 것이고,
Å(U+212B ANGSTROM SIGN)은 Å(U+00C5)와 같은 글리프입니다.

⅒(U+2152)는 IBM Plex Mono에 없어 만들었습니다. 그 글꼴의 분수는 분수 전용
글리프 `one.numr`, `fraction`, `숫자.dnom`을 −149 / 0 / +150에 놓는 구조라,
같은 부품을 씁니다. 분모가 두 자리이므로 `dnom`을 가로로 50%만 좁혀 나란히
둡니다. 위 첨자·아래 첨자(¹ ₁ ₀)로 만들면 분자가 다른 분수보다 11% 넓고
10% 높아집니다.

이중·삼중 적분 ∬(U+222C)과 ∭(U+222D)도 IBM Plex Mono의 ∫을 붙여 만듭니다.
CJK 소스의 것을 통째로 줄이면 ∫과 자형이 다른 데다 높이까지 낮아집니다
(줄이기 전 ∬은 높이가 ∫의 77%, ∭은 56%였습니다). ∫ 두 개와 세 개를 가로로만
눌러 붙이면 높이가 ∫과 같아집니다. JuliaMono와 Hack이 같은 방식이고, 칸을
채우는 비율(95%)도 그 둘(90~99%)과 맞습니다. 가로로만 누르므로 획은 각각
0.65배와 0.55배로 얇아집니다.

`narrowToHalf`는 소스가 어느 글꼴이든 결과 코드포인트 기준으로 적용되므로
레시피 최상위에 한 번만 적습니다. KR과 CJK가 같은 목록을 씁니다. 한쪽에 없는
코드포인트는 무시됩니다.

### 줄이고 나서 드러난 것

`narrowToHalf`는 잉크가 `maxInk`를 넘는 글자만 줄입니다. 그래서 두 가지를
따로 손봐야 했습니다.

**칸을 넘는 채로 남아 있던 16자.** 합성 단계에서 이미 반각 칸을 받았지만 크기는
줄지 않은 글자들입니다. 잉크가 590~610으로 528칸을 넘었고 1.0.0부터 그랬습니다.
옆 글자가 470으로 줄면서 드러났습니다. 전부 IBM Plex Sans JP에서 옵니다.

```
‿ ℏ ∉ ∟ ≃ ≅ ≢ ≲ ≳ ⊄ ⊅ ⊊ ⊋ ⊿ ⧺ ⧻
```

**작은 변형.** ∊(U+220A SMALL ELEMENT OF)는 원래 ∈의 0.64배인데, ∈만 470으로
줄고 ∊은 378이라 그대로 남아 둘의 크기가 같아졌습니다. ∈이 줄어든 비율(80%)을
∊에도 적용해 소스가 의도한 비율로 되돌렸습니다. 결과는 0.65 × 0.79로,
JuliaMono(0.64)나 Noto Sans Mono(0.75)와 같은 범위입니다.

## 둘러싼 연산자의 상자와 축

⊕ ⊖ ⊗ ⊘ ⊙ ⊞ ⊠은 크기와 높이가 서로 같아야 하는데 어긋나 있었습니다.
⊕⊖⊗는 Sans JP, ⊙은 Sans KR에서 와서 ⊙만 세로로 529(나머지는 470)였고,
세로 중심도 332부터 380까지 흩어졌습니다.

더 큰 문제는 기준선입니다. 일곱 자 모두 수학 축보다 46~94 위에 떠 있었습니다.
수학 축은 `+`의 자면 중심으로, IBM Plex Mono에서 두께가 달라도 286.7~287.2로
일정합니다. JuliaMono, Hack, Noto Sans Mono, MS Gothic, Sarasa는 모두 이
기호들을 `+`와 같은 축에 놓습니다(오차 0~2).

`alignBox`로 잉크를 470 정사각에 맞추고 세로 중심을 287로 옮겼습니다. 소스가
여러 글꼴로 갈리는 계열이라 소스별 `ops`가 아니라 레시피 최상위에 적고 모든
소스에 같은 규칙을 적용합니다. `size`에 숫자 하나를 주면 정사각형이고,
`[가로, 세로]`를 주면 그 상자입니다.

### 축에 올린 기호

둘러싼 연산자 말고도 스물한 자가 수학 축보다 18~104 위에 떠 있었습니다.
IBM Plex Sans KR·JP·SC가 이 기호들을 CJK 본문 기준으로 그리기 때문입니다.
`alignBox`로 내렸습니다.

```
∅ ∦ ∧ ∨ ∩ ∪ ∴ ∵ ∷ ∼ ∽ ∝ ≃ ≅ ≌ ≒ ≡ ≮ ≯ ⊥ ⋯
```

∝(U+221D)은 ∞와 짝이 되는 자형인데 52 위에 있었습니다. ∞는 IBM Plex Mono의
것이고 여덟 두께 모두 높이 337, 중심 287로 일정해서, 그 값이 곧 축입니다.

넣지 않은 것도 있습니다. ∀ ∃는 대문자처럼 쓰여 참고 글꼴도 위에 둡니다
(JuliaMono +88, Hack +51, Sarasa +28). ∑ ∏ √ ∫처럼 세로로 긴 글자는 축 개념이
다릅니다. ⊊ ⊋ ≲ ≳처럼 아래에 표가 붙는 글자는 자면 중심으로 맞추면 윗부분이
올라가므로 그대로 두었습니다.

### 도형

도형 40자도 같은 문제가 있었습니다. 세로 축은 39자가 벗어나 있었고, 그것도
한 무리가 아니었습니다. Sans KR에서 온 27자는 470×528(비율 0.89)에 축 +48,
Sans JP에서 온 12자는 470×470(비율 1.00)에 축 +92로, 한 블록 안에 두 체계가
섞여 있었습니다. ■과 ▢, ●과 ◯를 나란히 놓으면 크기도 높이도 달랐습니다.

큰 도형 37자를 470 정사각으로 맞추고 40자 전부를 축에 올렸습니다. 참고 글꼴의
■ 비율은 0.93~1.00이고, JuliaMono와 Sarasa는 축에 맞춥니다.

곁들여 둘을 더 고쳤습니다. ▶ ▷ ◀ ◁는 470×637로 ▲ ▽(470×460)보다 109
컸습니다. 이제 같습니다. ▪ ▫은 ■의 0.75배였습니다. ■이 71%로 줄 때 이들은
470을 넘지 않아 그대로 남은 탓입니다. 참고 글꼴은 0.51~0.70이라 0.6배(282)로
맞췄습니다.

비율이 고유한 셋은 크기를 그대로 두고 축만 맞췄습니다. ▱는 평행사변형,
◊는 마름모(Plex Mono), ◦는 ○의 0.5배입니다.

가로 정렬은 원래부터 문제가 없었습니다. 40자 모두 좌우 여백이 같습니다.

## 글자는 IBM Plex Mono에서만

그리스, IPA 확장, 결합 분음 기호, 라틴 확장은 IBM Plex Mono에서만 가져옵니다.
CJK 소스에도 있지만 쓰지 않습니다.

CJK 글꼴의 그리스는 전각 칸에 맞춰 그린 것이라 Plex Mono의 라틴과 크기가
다릅니다.

| | 대문자 높이 | 잉크 폭 |
|---|---|---|
| 라틴 A (Plex Mono) | 656 | 475 |
| 그리스 Α (Plex Sans JP) | 733 | 620 |
| 키릴 А (Plex Mono) | 656 | 475 |

대문자가 12% 높고 31% 넓습니다. 반각 칸 528을 그리스 32자, IPA 12자가 넘었고,
한 알파벳 안에서 Μ Φ Ψ ϐ ϑ ϕ ϛ 일곱 자만 전각이었습니다. 키릴은 Plex Mono가
192자를 모두 갖고 있어 이런 일이 없습니다.

결합 분음 기호는 더 나쁩니다. Plex Mono에서 온 22자는 폭이 0이라 앞 글자에
얹히는데, CJK 소스에서 온 26자는 폭이 반각이나 전각이라 결합하지 않고 제 칸을
차지했습니다. 둘러싸는 결합 기호 ⃝ ⃞도 같습니다.

라틴 확장도 마찬가지입니다. Plex Mono가 라틴 확장 B를 38자 중 33자, 라틴 확장
추가를 127자 중 125자 갖고 있는데, 남는 몇 자를 CJK 소스에서 채우면 Ɠ는 잉크가
704로 칸을 넘고 Ḿ ḿ는 전각으로 들어옵니다.

수식 문자 ʰ ʲ ʷ ˁ ˡ 다섯 자는 Plex Mono에 없어 `finalize.removeCodepoints`로
뺐습니다. 라틴 글자를 작게 올린 모양이라 Plex Mono의 자형과 섞입니다.

없는 글자는 시스템 글꼴로 넘어갑니다. 자형이 섞이고 칸이 어긋나는 것보다
낫다고 보았습니다.

## 둘러싼 숫자를 하나로

① ⑴ Ⓐ ⓐ ❶ 은 한 계열입니다. 그런데 두 겹 원 ⓵–⓾ 만 IBM Plex Sans JP에서 오고
나머지는 Sans KR에서 옵니다.

| | 잉크 | 세로 중심 |
|---|---|---|
| ❶ ① Ⓐ ⓐ (Sans KR) | 818 × 920 | 335 |
| ⓵ ⓶ ⓷ (Sans JP) | 922 × 922 | 380 |
| ⑴ ⒜ 🄐 (Sans KR) | 854 × 898 | 311 |

같은 줄에 놓으면 ⓵ 만 크고 높이 떠 보입니다. `alignBox`로 176자를 ❶ 에
맞췄습니다. 원이 정원이 아니라서 `size`에 `[818, 920]`처럼 가로세로를 따로
줍니다.

숫자에 마침표를 찍은 ⒈–⒛ 과 🄀 은 여기서 뺐습니다. 블록 이름이 "둘러싼
영숫자"지만 이 스물한 자는 둘러싸인 글자가 아닙니다. U+1F100의 정식 이름부터
DIGIT ZERO FULL STOP입니다.

| | 잉크 | 아래끝 | 위끝 |
|---|---|---|---|
| ❶ ① 🄐 | 818 × 920 | −125 | 795 |
| ⒈ 🄀 | 약 740 × 715 | −12 | 약 700 |

원과 괄호는 베이스라인 아래 −125까지 내려가 글자를 감쌉니다. 마침표 형태는
감싸는 것이 없는 맨 숫자라 베이스라인 위에 섭니다. 920으로 늘리면 안의 숫자가
656에서 850으로 커져 맨 숫자보다 커지고, 아래끝이 −125로 내려가 숫자가
베이스라인 밑으로 파고듭니다.

이 스물한 자는 이미 자기들끼리 맞아 있습니다. 전부 아래끝이 −12로 같고, 높이가
698과 710으로 갈리는 것은 둥근 숫자의 오버슛입니다. ⒈ ⒋ ⒌ ⒍ ⒎ 처럼 윗변이
평평한 숫자만 든 것이 698, ⒉ ⒊ ⒑ ⒙ 처럼 0 2 3 8 9가 든 것이 710입니다.

## 2em 과 3em 대시

⸺ (U+2E3A)과 ⸻ (U+2E3B)은 이름이 폭을 말하는 글자입니다. 그런데 둘 다 전각
한 칸에 잉크 836과 850으로 들어 있어, 2em도 아니고 서로 구별되지도 않았습니다.

이 글꼴에서 em 대시 — (U+2014)는 반각 528을 0부터 528까지 꽉 채웁니다. 그래서
`——`를 이어 쓰면 선이 끊기지 않습니다. 그 성질을 그대로 써서 `compose`로 둘을
붙여 1056, 셋을 붙여 1584로 만들었습니다. 반각 두 칸과 세 칸입니다.

## 계열의 소스를 맞추는 것

한 계열의 글자를 소스 글꼴이 나눠 가지면 자형이 섞입니다. 부분집합
⊂⊃⊄⊅⊆⊇⊊⊋에서 IBM Plex Sans KR은 ⊂⊃⊆⊇ 넷만 갖고 있어 나머지가
Sans JP에서 왔고, 원소 ∈∉∊∋도 둘씩 갈렸습니다. KR의 ⊂는 687×500으로 납작하고
JP의 ⊂는 590×540이라, 가로를 470으로 맞추면 세로가 384와 668로 벌어집니다.

Sans JP가 두 계열을 모두 갖고 있으므로 KR이 양보하도록 `dropRanges`에 적었습니다
(Monoplex CJK만 해당합니다. Monoplex KR에는 JP 소스가 없어 계열이 이미 KR
자형으로 통일되어 있습니다). 가나를 Meiryo로 넘길 때 쓴 것과 같은 방법입니다.

원 연산자 ⊕⊖⊗⊘⊙와 합동·근사 ≅≈≌≒는 한 소스가 전부 갖고 있는 곳이 없어
그대로 두었습니다.

## U+274C를 지운 이유

IBM Plex Mono의 U+274C CROSS MARK는 흑백 반각 글리프입니다. 이것이 있으면
OS의 이모지 글꼴로 폴백하지 않아 ❌가 흑백으로 나옵니다. 지워서
폴백시킵니다. 상류 PlemolJP(be3ac3a)의 수정입니다.

## 박스 드로잉

U+2500–259F 160자는 반각으로 손질한 `source/AdjustedGlyphs/Box_Drawing_half.sfd`
에서 가져옵니다. IBM Plex Mono와 IBM Plex Sans KR 양쪽에 원본이 있지만
전각이거나 칸에 맞지 않습니다.

## 굵기마다 커버리지가 다른 곳

IBM Plex Sans KR 1.002는 **Thin 굵기에만 57자가 없습니다.**

```
U+3008 U+3009  U+FB00 U+FB03 U+FB04  U+FF5E
U+FFA1–FFDC (반각 한글 자모 51자)
```

나머지 일곱 굵기에는 다 있습니다. 그래서 Monoplex KR의 Thin은 13,054자,
나머지는 13,111자입니다. 0.0.2도 이 57자의 차이는 같았습니다.

IBM이 1.003에서 이 57자를 전 굵기에서 빼는 쪽으로 정리했는데, 그래서
1.003으로 올리지 않았습니다.

## IBM Plex Sans KR을 1.003으로 올리지 않은 이유

자면은 사실상 같습니다. bbox가 12,151/12,156자 완전 일치입니다. 그런데
반각 한글 자모를 비롯한 57자가 빠지고 16자만 늘어납니다. 늘어난 16자는
이미 IBM Plex Mono 쪽에 있어서 쓰이지 않으므로, 얻는 것 없이 잃기만
합니다.

## 글꼴이 스스로 들고 다니는 라이선스

`output` 의 `copyright`, `license`, `licenseURL`, `trademark` 는 글꼴 파일의
name 테이블 0, 13, 14, 7 번 칸에 그대로 들어갑니다. ttf 하나만 따로 설치되면
저장소의 [LICENSE.md](LICENSE.md) 가 따라가지 않으므로 글꼴 자신이 들고 있어야
합니다. OFL 2 항이 각 사본에 저작권 표시와 라이선스를 담으라고 하고, 그 방법으로
기계가 읽을 수 있는 메타데이터 칸을 인정합니다.

IBM 쪽 문구는 소스 글꼴의 `LICENSE.txt` 첫 줄과 name 13 번을 그대로 옮긴
것입니다. 지어낸 문장이 아닙니다.

PlemolJP 를 적는 이유는 U+2500–259F 괘선과 블록입니다.
`source/AdjustedGlyphs/Box_Drawing_half.sfd` 헤더에는 `FontName: IBMPlexSansJP`
라고 적혀 있지만, 그 안의 윤곽은 IBM 것이 아닙니다.

| U+2500 | 폭 | 획 두께 |
|---|---|---|
| IBM Plex Sans JP | 1000 | 40 |
| Box_Drawing_half.sfd | 600 | 68 |

헤더는 그 글꼴의 작업 사본에서 저장했기 때문에 남은 것이고, 글리프는 다시 그린
것입니다. 그래서 PlemolJP 의 저작권자를 적습니다.

Nerd Fonts 쪽은 `variantOutput` 으로 Nerd Font 판에만 붙입니다. 그 판에만
아이콘 글리프가 들어가기 때문입니다. 아이콘 묶음들은 각자의 저작권자와
라이선스를 가지며, Nerd Fonts 배포본이 그것을 한데 정리해 두지 않았으므로
그렇게 적습니다. 상업적 이용이 제한되는 Pomicons(U+E000–E00A)는 `keepRanges`
에서 빼 두었습니다.

빌드한 글꼴에서 확인할 수 있습니다.

```
Monoplex KR          name 0  Kim Yangsu / IBM Corp. / Yuko Otawara
                     name 7  IBM Plex® is a trademark of IBM Corp, ...
                     name 13 SIL Open Font License, Version 1.1 ...
                     name 14 http://scripts.sil.org/OFL
Monoplex KR Nerd Font  위에 더해
                     name 0  Ryan L McIntyre 한 줄이 붙고
                     name 13 아이콘 글리프에 관한 문장이 붙는다
```

## 소스 글꼴 버전

버전은 글꼴 파일 안의 값(name ID 5)입니다. 배포 채널의 패키지 번호와는
다를 수 있습니다. 예를 들어 `@ibm/plex-mono@2.5.0` 릴리스에 들어 있는
글꼴의 내부 버전은 `2.005`입니다.

| 글꼴 | 내부 버전 | 업스트림 릴리스 |
|---|---|---|
| IBM Plex Mono | 2.005 | [`@ibm/plex-mono@2.5.0`](https://github.com/IBM/plex/releases/tag/%40ibm%2Fplex-mono%402.5.0) |
| IBM Plex Sans KR | 1.002 | [`v6.4.2`](https://github.com/IBM/plex/releases/tag/v6.4.2) (마지막 통합 릴리스) |
| IBM Plex Sans JP | 1.004 | [`@ibm/plex-sans-jp@3.0.0`](https://github.com/IBM/plex/releases/tag/%40ibm%2Fplex-sans-jp%403.0.0) |
| IBM Plex Sans TC | 1.001 | [`@ibm/plex-sans-tc@1.1.1`](https://github.com/IBM/plex/releases/tag/%40ibm%2Fplex-sans-tc%401.1.1) |
| IBM Plex Sans SC | 1.000 | [`@ibm/plex-sans-sc@1.1.0`](https://github.com/IBM/plex/releases/tag/%40ibm%2Fplex-sans-sc%401.1.0) |
| Blex Mono Nerd Font | Nerd Fonts 3.5.1 | [`v3.5.1`](https://github.com/ryanoasis/nerd-fonts/releases/tag/v3.5.1) |

## 커버리지

| | Monoplex KR | KR Nerd Font | Monoplex CJK | CJK Nerd Font |
|---|---|---|---|---|
| cmap | 13,111 | 23,979 | 44,397 | 55,265 |
| 한글 음절 | 11,172 | 11,172 | 11,172 | 11,172 |
| 한글 자모 | 94 | 94 | 94 | 94 |
| 반각 한글 자모 | 51 | 51 | 51 | 51 |
| 통합한자 | — | — | 20,992 | 20,992 |
| 확장 A | — | — | 6,592 | 6,592 |
| 호환한자 | — | — | 467 | 467 |
| KS X 1001 한자 | — | — | 4,888 / 4,888 | 4,888 / 4,888 |
| 히라가나 / 가타카나 | — | — | 93 / 96 | 93 / 96 |
| 반각 가타카나 | — | — | 58 | 58 |
| 박스 드로잉 | 160 | 160 | 160 | 160 |
| Powerline | — | 40 | — | 40 |
| Material Design Icons | — | 6,896 | — | 6,896 |
| Codicons | — | 540 | — | 540 |
| Devicons | — | 601 | — | 601 |
| 16스타일 용량 | 42 MB | 77 MB | 176 MB | 210 MB |

한글 음절 11,172자는 U+AC00–D7A3 전부입니다.
