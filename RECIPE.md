...
이슈 #2의 정리입니다. 수학 기호와 도형, 화살표 같은 기호 블록에서 반각과
전각이 섞여 있었습니다. IBM Plex Mono가 가진 기호는 반각, IBM Plex Sans KR·JP·
TC·SC가 가진 기호는 그 글꼴이 전각으로 그리므로 전각으로 들어왔기
때문입니다. 터미널(xterm.js, Windows Terminal)은 East Asian Width가 W나 F가
아닌 글자를 1칸으로 세므로, 전각 글리프는 옆 칸을 침범합니다.

글자마다 검토해서 셋으로 나눴습니다. 결정 근거로 다른 고정폭 글꼴 13종
(나눔고딕코딩, D2Coding, JetBrains Mono, Liberation Mono, Cascadia Code, Hack,
Cousine, Fira Code, Consolas, JuliaMono, Source Code Pro, IBM Plex Mono,
Sometype Mono)의 폭을 함께 봤습니다.

| 결정 | 글자 수 | 처리 |
|---|---|---|
| 반각 | 110 | `narrowToHalf`. 잉크 폭이 470을 넘으면 가로세로 같은 비율로 줄이고 반각에 가운데 놓는다. 겹문장부호 5자는 아래처럼 조합 |
| 전각 | 10 | ❶–❿ (U+2776–277F). 그대로 둔다 |
| 제거 | 143 | `finalize.removeCodepoints`. 로마 숫자, 기타 기술(⌘, 괄호 조각), 기타 기호(★ ♥ ☎ 등), 딩뱃 일부, 보충 화살표 |

기준 470은 IBM Plex Mono 자체 반각 기호의 잉크 폭(중앙값 436, 상위 5% 513)에서
잡았습니다. 원 모양(⊙ ○ ◎)은 가로만 누르면 뜻을 잃으므로 같은 비율로
줄입니다. ⊙은 62%가 됩니다.

겹문장부호 ‼ ⁇ ⁈ ⁉와 ‥는 예외입니다. 통째로 줄이면 크기가 제각각이고
너무 작아져서, IBM Plex Mono의 `!` `?` `.`를 붙여 만듭니다(`compose` op).
‼는 `!` 두 개를 그대로, ⁇는 `?`를 가로로만 58%로 좁혀 두 개, ⁈와 ⁉는
65%로 좁힌 `?`와 그대로의 `!`, ‥는 `…`의 점 간격으로 `.` 두 개입니다. 높이가
본문 부호와 같아집니다. Hack이 같은 방식이고, Plex Mono의 `?`가 Hack보다
넓어 비율을 더 줄였습니다.

같은 방법으로 다섯 자를 IBM Plex Mono에서 만듭니다. ‵(U+2035)는 ′를 좌우로,
∇(U+2207)는 ∆(U+2206)를 상하로, ℧(U+2127)는 Ω(U+2126)를 상하로 뒤집은 것이고,
Å(U+212B ANGSTROM SIGN)은 Å(U+00C5)와 같은 글리프이며, ⅒(U+2152)는 ⅐(U+2150)
방식으로 위 첨자 1, 분수 사선, 가로로 55% 좁힌 아래 첨자 1 0을 붙인
것입니다.

`narrowToHalf`는 소스가 어느 글꼴이든 결과 코드포인트 기준으로 적용되므로
레시피 최상위에 한 번만 적습니다. KR과 CJK가 같은 목록을 씁니다. 한쪽에 없는
코드포인트는 무시됩니다.
